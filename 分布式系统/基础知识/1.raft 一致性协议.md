...
- [8 Raft与Multi-Paxos的异同](#8-raft与multi-paxos的异同)
    - [8.1 Raft与Multi-Paxos中相似的概念](#81-raft与multi-paxos中相似的概念)
    - [8.2 Raft与Multi-Paxos的不同](#82-raft与multi-paxos的不同)
...
![2019-10-10-16-41-46.png](./images/2019-10-10-16-41-46.png)
...
增加上述限制后，即使日志（2，2）已经被大多数节点（S1、S2、S3）确认了，但是它不能被提交，因为它是来自之前term（2）的日志，直到S1在当前term（4）产生的日志（4， 4）被大多数Followers确认，S1方可提交日志（4，4）这条日志，当然，根据Raft定义，（4，4）之前的所有日志也会被提交。此时即使S1再下线，重新选主时S5不可能成为Leader，因为它没有包含大多数节点已经拥有的日志（4，4）。(所以上面的第二条限制才是关键，提交日志方式是通过batch方式，而不是一条条提交)
...
![2019-10-10-16-58-20.png](./images/2019-10-10-16-58-20.png)
...
![2019-10-10-17-01-20.png](./images/2019-10-10-17-01-20.png)
...
* 如果Cold和Cnew中的两个多数派确认了Cold U Cnew这条日志，Leader就提交这条log entry；(关键：很有意思是必须要两个多数派同意这条日志才提交这个日志，日志提交后，这样最终选举leader一定是所有机器中的多数成员)
...
## 8.1 Raft与Multi-Paxos中相似的概念
![2019-10-10-17-17-27.png](./images/2019-10-10-17-17-27.png)

## 8.2 Raft与Multi-Paxos的不同
![2019-10-10-17-17-59.png](./images/2019-10-10-17-17-59.png)
